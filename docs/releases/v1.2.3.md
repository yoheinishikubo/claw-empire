...
- Messenger session tokens are encrypted at rest using AES-256-GCM with `OAUTH_ENCRYPTION_SECRET` (legacy fallback: `SESSION_SECRET`) and decrypted only for runtime delivery.
