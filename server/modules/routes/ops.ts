...
import type { CliUsageEntry, DecryptedOAuthToken } from "./shared/types.ts";
...
export function registerRoutesPartC(ctx: RuntimeContext): RouteOpsExports {
  ...
  app.get("/api/stats", (_req, res) => {
    ...
    const inboxTasks = (db.prepare("SELECT COUNT(*) as cnt FROM tasks WHERE status = 'inbox'").get() as { cnt: number })
      .cnt;
    const plannedTasks = (
      db.prepare("SELECT COUNT(*) as cnt FROM tasks WHERE status = 'planned'").get() as { cnt: number }
    ).cnt;
    const reviewTasks = (
      db.prepare("SELECT COUNT(*) as cnt FROM tasks WHERE status = 'review'").get() as { cnt: number }
    ).cnt;
    const cancelledTasks = (
      db.prepare("SELECT COUNT(*) as cnt FROM tasks WHERE status = 'cancelled'").get() as { cnt: number }
    ).cnt;
    const collaboratingTasks = (
      db.prepare("SELECT COUNT(*) as cnt FROM tasks WHERE status = 'collaborating'").get() as { cnt: number }
    ).cnt;
    ...
    const idleAgents = (db.prepare("SELECT COUNT(*) as cnt FROM agents WHERE status = 'idle'").get() as { cnt: number })
      .cnt;
  ...
  function buildTerminalProgressHints(
    raw: string,
    maxHints = 14,
  ): {
  ...
  app.put("/api/api-providers/:id", (req: any, res: any) => {
    ...
    if ("base_url" in body && body.base_url) {
