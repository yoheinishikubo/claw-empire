...
function resolveCustomSkillDirectory(customSkillsDir: string, skillName: string): {
  dirName: string;
  dirPath: string;
...
app.post("/api/skills/custom", async (req, res) => {
  try {
    ...
    if (!parsedSkillName) {
      return res.status(400).json({ error: "invalid skillName format (alphanumeric, dash, underscore, max 80 chars)" });
    }
    if (!content) {
    ...
    for (const provider of providers) {
      ...
      try {
        ...
        `).run(
          ...
          `custom/${canonicalSkillName}`, canonicalSkillName, skillName,
...
app.get("/api/skills/custom", (_req, res) => {
  try {
    ...
    const entries = fs.readdirSync(customSkillsDir, { withFileTypes: true });
    const dedupedByCanonical = new Map<string, {
      skillName: string;
      providers: string[];
      createdAt: number;
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        const canonicalSkillName = String(meta.canonicalSkillName ?? resolvedSkillName).trim().toLowerCase();
...
app.delete("/api/skills/custom/:skillName", (req, res) => {
  try {
    ...
    db.prepare(`DELETE FROM skill_learning_history WHERE lower(repo) = lower(?)`).run(`custom/${resolvedSkill.canonicalName}`);
