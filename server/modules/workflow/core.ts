...
import { createPromptSkillsHelper } from "./core/prompt-skills.ts";
...
export function initializeWorkflowPartA(ctx: RuntimeContext): WorkflowCoreExports {
  ...
  function mergeWorktree(
    ...
    try {
      ...
      try {
        ...
        /* proceed with merge attempt anyway */
      }
      ...
      try {
        ...
        /* ignore conflict detection failure */
      }
  ...
  function normalizeConversationReply(raw: string, maxChars = 420, opts: { maxSentences?: number } = {}): string {
    ...
    text = text
      ...
      .replace(
        /\b(I need to|Let me|I'll|I will|First, I'?ll)\b[^.!?。！？]{0,140}\b(analy[sz]e|examin|inspect|check|review|look at)\b[^.!?。！？]*[.!?。！？]?/gi,
        " ",
      )
      .replace(
        /\b(current codebase|relevant files|quickly examine|let me quickly|analyze the current project)\b[^.!?。！？]*[.!?。！？]?/gi,
        " ",
      )
      .replace(/```[\s\S]*?```/g, " ")
      .replace(/`[^`]*`/g, " ")
  ...
  function formatMeetingTranscript(transcript: MeetingTranscriptEntry[], lang: Lang = getPreferredLanguage()): string {
