...
export function initializeWorkflowPartC(ctx: RuntimeContext): WorkflowOrchestrationExports {
  ...
  function isPresentationReportTask(
    task: { task_type?: string | null; description?: string | null } | null | undefined,
  ): boolean {
  ...
  function extractReportDesignParentTaskId(
    task:
      | {
  ...
  function startPlannedApprovalMeeting(
    ...
    void (async () => {
      ...
      try {
        ...
        const planningLeader = leaders.find((l: AgentRow) => l.department_id === "planning") ?? leaders[0];
        const otherLeaders = leaders.filter((l: AgentRow) => l.id !== planningLeader.id);
        let hasSupplementSignals = false;
        const seatIndexByAgent = new Map(leaders.slice(0, 6).map((leader: AgentRow, idx: number) => [leader.id, idx]));
