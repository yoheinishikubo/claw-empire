...
- **视频前期渲染链路稳定化** - 强化 `video_preprod` 的 `VIDEO_FINAL_RENDER` 流程，覆盖计划阶段种子创建、延后委派、stale 状态修复与重复触发防护。
- **办公室 Pack 首次 hydration 与持久化** - 首次进入 Pack 时注入种子数据，hydrated 后切换为 DB 持久模式，稳定保留用户自定义（含 provider 修改）。
- **报告输出策略升级（HTML + PPTX）** - 报告办公室默认同时产出 HTML 与 PPTX；`python-pptx` 仅在 PPT Team 不可用/硬失败时作为回退方案。
- **既有运行时稳定性 Fix 集合** - 补强 worktree 分支冲突恢复、Claude `--no-tools` 参数保留，以及 YOLO/WebSocket 重试稳定性。
