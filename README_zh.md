...
<p align="center">
  ...
  <a href="docs/releases/v2.0.3.md">发布说明</a> &middot;
...
- **现在可以在合并前直接看到最终分支校验结果** - Diff Modal 会展示 `GET /api/tasks/:id/verify-commit` 的 verdict、比较基准 ref、提交数、变更文件和未提交文件状态。
- **完成报告会保留合并前的校验证据** - 手动合并成功时会写入 `Final branch verification: ...` 日志，并在报告弹窗的规划摘要区域展示。
- **即使当前活跃代理列表变化，报告头像仍保持 sprite 脸图** - 当当前 `agents` 数组中找不到负责人时，界面会根据报告 payload 重建 fallback agent，而不是退化成 emoji。
- **本次只选择性吸收了 PR #54 中安全的部分** - 包含 worktree verification API/UI、`scripts/cleanup-staff.mjs` 以及可选的 `deploy/` self-host 模板，不包含 task model 变更或 admin/local-server 扩展。

- 详细说明: [`docs/releases/v2.0.3.md`](docs/releases/v2.0.3.md)
