    ...
    <div className="space-y-4 p-6">
          ...
          >
            {disconnecting
              ...
              : reason === 'not_connected'
                ...
                : t({ ko: 'GitHub 재연결 (repo 권한)', en: 'Reconnect GitHub (repo scope)', ja: 'GitHub 再接続 (repo 権限)', zh: '重新连接 GitHub（repo 权限）' })}
          </button>
      ...
      {deviceStatus === 'waiting' && deviceUserCode && (
        <div className="space-y-3 rounded-xl border border-blue-500/30 bg-blue-900/20 p-4">
          ...
          {deviceVerifyUrl && (
            ...
            >
              {t({ ko: 'GitHub 인증 페이지 열기', en: 'Open GitHub verification page', ja: 'GitHub 認証ページを開く', zh: '打开 GitHub 验证页面' })}
            </a>
          )}
      ...
      {deviceStatus === 'complete' && (
        <div className="rounded-lg border border-green-500/20 bg-green-500/10 px-3 py-2 text-xs text-green-400">
          {t({ ko: 'GitHub 연결 완료! 리포 목록을 불러옵니다...', en: 'GitHub connected! Loading repositories...', ja: 'GitHub 接続完了！リポジトリを読み込みます...', zh: 'GitHub 已连接！正在加载仓库...' })}
        </div>
      )}
  ...
  if (statusLoading) {
    return (
      <div className="flex items-center justify-center p-8">
        <p className="text-sm text-slate-400">{t({ ko: '확인 중...', en: 'Checking...', ja: '確認中...', zh: '检查中...' })}</p>
      </div>
    );
  }
      ...
      <div className="flex-1 overflow-y-auto p-5">
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-xl border border-slate-600/50 bg-slate-800/40 p-3 space-y-2">
              <p className="text-xs font-medium text-slate-300">
                {t({ ko: '직접 입력 (Private 리포 포함)', en: 'Direct Input (incl. private repos)', ja: '直接入力（プライベートリポ含む）', zh: '直接输入（含私有仓库）' })}
              </p>
              <div className="flex gap-2">
                <input
                  type="text"
                  placeholder={t({ ko: 'owner/repo 또는 GitHub URL', en: 'owner/repo or GitHub URL', ja: 'owner/repo または GitHub URL', zh: 'owner/repo 或 GitHub URL' })}
                  value={directInput}
                  onChange={(e) => { setDirectInput(e.target.value); setDirectInputError(null); }}
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[11px] text-slate-500">{t({ ko: '또는 목록에서 선택', en: 'or select from list', ja: 'またはリストから選択', zh: '或从列表选择' })}</span>
              <div className="flex-1 border-t border-slate-700" />
            </div>

            <input
              type="text"
              placeholder={t({ ko: '리포지토리 검색...', en: 'Search repositories...', ja: 'リポジトリを検索...', zh: '搜索仓库...' })}
              value={repoSearch}
            ...
            />
            {reposLoading ? (
              <p className="text-xs text-slate-400">{t({ ko: '불러오는 중...', en: 'Loading...', ja: '読み込み中...', zh: '加载中...' })}</p>
            ) : repos.length === 0 ? (
              <p className="text-xs text-slate-500">{t({ ko: '검색 결과 없음', en: 'No results', ja: '結果なし', zh: '无结果' })}</p>
            ) : (
                ...
                {repos.map((repo) => (
                  ...
                  >
                    ...
                    <p className="mt-1 text-[11px] text-slate-500">
                      {t({ ko: '기본 브랜치', en: 'Default', ja: 'デフォルト', zh: '默认分支' })}: {repo.default_branch} · {new Date(repo.updated_at).toLocaleDateString()}
                    </p>
                  </button>
                ))}
              </div>
        ...
        {step === 'branch' && selectedRepo && (
          <div className="space-y-3">
                ...
                <div className="rounded-xl border border-amber-500/30 bg-amber-900/10 p-3 space-y-2">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onKeyDown={(e) => { if (e.key === 'Enter' && patToken.trim()) void handlePatRetry(); }}
            ...
            {branchesLoading ? (
              ...
              <p className="text-xs text-slate-500">{t({ ko: '브랜치 없음', en: 'No branches', ja: 'ブランチなし', zh: '无分支' })}</p>
            ) : (
              <div className="space-y-1">
                {branches.map((b) => (
                  <button
        ...
        {step === 'clone' && selectedRepo && selectedBranch && (
          <div className="space-y-4">
            ...
            <label className="block text-xs text-slate-400">
              {t({ ko: '핵심 목표 (선택)', en: 'Core Goal (optional)', ja: 'コア目標（任意）', zh: '核心目标（可选）' })}
              <textarea
                rows={3}
                value={coreGoal}
            ...
            <div className="flex gap-2">
              ...
              >
                {creating
                  ...
                  : t({ ko: 'GitHub에서 가져오기', en: 'Import from GitHub', ja: 'GitHub からインポート', zh: '从 GitHub 导入' })}
              </button>
              <button
                type="button"
                onClick={() => { setStep('branch'); setCloneStatus('idle'); setCloneError(null); setCreating(false); }}
                disabled={creating}
