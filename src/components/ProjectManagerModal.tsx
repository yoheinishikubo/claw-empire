  ...
  return (
    ...
    >
      <div className="flex h-[86vh] w-[min(1180px,95vw)] flex-col overflow-hidden rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl md:flex-row">
        <aside
          className={`flex w-full flex-col border-r border-slate-700 bg-slate-900/70 md:w-[330px] ${selectedProjectId || isCreating || githubImportMode ? "hidden md:flex" : "flex"}`}
        >
                        ...
                        <div className="rounded-xl border border-slate-700 bg-slate-900/50 p-3 space-y-2">
                          <div className="flex items-center justify-between">
                            ...
                            {departments.length > 0 && (
                              ...
                              >
                                <option value="all">
                                  {t({ ko: "전체 부서", en: "All Depts", ja: "全部署", zh: "所有部门" })}
                                </option>
                                {departments.map((d) => (
                                  <option key={d.id} value={d.id}>
                                    {d.icon} {language === "ko" ? d.name_ko || d.name : d.name}
                                  </option>
                                ))}
                              </select>
                            )}
                          ...
                          <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
                            ...
                            <span className="rounded-full border border-amber-500/30 bg-amber-500/10 px-2 py-0.5 text-amber-300">
                              {t({ ko: "팀장", en: "Leaders", ja: "リーダー", zh: "组长" })}:{" "}
                          ...
                          <div className="max-h-48 overflow-y-auto space-y-1 pr-1">
                            {agents
                              ...
                              .sort((a, b) => {
                                ...
                                return (
                                  (roleOrder[a.role] ?? 9) - (roleOrder[b.role] ?? 9) || a.name.localeCompare(b.name)
                                );
                              })
                              .map((agent) => {
                    ...
                    {!selectedProject ? (
                      ...
                      <div className="mt-2 max-h-56 overflow-x-hidden overflow-y-auto space-y-2 pr-1">
                        {groupedTaskCards.map((group) => (
                          ...
                          >
                            <p className="whitespace-pre-wrap break-all text-xs font-semibold text-slate-100">
                              {group.root.title}
                            </p>
                            ...
                            <p className="mt-1 text-[11px] text-blue-300">
                              {t({ ko: "하위 작업", en: "Sub tasks", ja: "サブタスク", zh: "子任务" })}:{" "}
                            ...
                            {group.children.length > 0 && (
                              <div className="mt-1 space-y-1">
                                {group.children.slice(0, 3).map((child) => (
                                  <p
                                    key={child.id}
                                    className="whitespace-pre-wrap break-all text-[11px] text-slate-500"
                                  >
                                    - {child.title}
                                  </p>
                                ))}
                        ...
                        {sortedReports.map((row) => (
                          ...
                          >
                            <div className="min-w-0">
                              <p className="whitespace-pre-wrap break-all text-xs font-medium text-slate-100">
                                {row.title}
                              </p>
                        ...
                        {sortedDecisionEvents.map((event) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <p className="mt-1 whitespace-pre-wrap break-all text-[11px] text-slate-300">
                                {event.summary}
                              </p>
          ...
          >
            ...
            <div className="space-y-2 px-4 py-4">
              ...
              <div className="rounded-md border border-slate-700 bg-slate-800/60 px-3 py-2 text-[11px] text-slate-300">
                <p>
                  {t({ ko: "선택 요약", en: "Selection Summary", ja: "選択サマリー", zh: "选择摘要" })}:{" "}
                  {manualSelectionStats.total}
                </p>
                <p>
