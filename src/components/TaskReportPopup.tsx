      ...
      <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-3">
        <div className="mb-1 flex items-center justify-between gap-2">
          <p className="text-xs font-semibold text-emerald-300">
            {t({ ko: '기획팀장 최종 취합본', en: 'Planning Lead Consolidated Summary', ja: '企画リード統合サマリー', zh: '规划负责人汇总摘要' })}
          </p>
        ...
        <pre className="max-h-[60vh] overflow-auto whitespace-pre-wrap text-xs leading-relaxed text-emerald-100">
          {planningSummary?.content || t({ ko: '요약 내용이 없습니다', en: 'No summary text', ja: 'サマリーなし', zh: '暂无摘要内容' })}
        </pre>
      </div>
      <div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
      ...
      >
          ...
          <div className="min-w-0">
            <div className="mb-1 flex items-center gap-2">
              ...
              <h2 className="truncate text-lg font-bold text-white">
                {t({ ko: '작업 완료 보고서', en: 'Task Completion Report', ja: 'タスク完了レポート', zh: '任务完成报告' })}
              </h2>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-slate-400">
                ...
                <span>{t({ ko: '완료', en: 'Completed', ja: '完了', zh: '完成' })}: {fmtTime(currentReport.task.completed_at)}</span>
        ...
        <div className="max-h-[68vh] overflow-y-auto px-6 py-4">
          {activeTab === 'planning' ? renderPlanningSummary() : selectedTeam ? renderTeamReport(selectedTeam) : (
            <p className="text-sm text-slate-500">
              {t({ ko: '표시할 보고서가 없습니다', en: 'No report to display', ja: '表示するレポートがありません', zh: '没有可显示的报告' })}
            </p>
          )}
        </div>
